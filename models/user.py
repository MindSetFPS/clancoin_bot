...
class User():
    ...
    def set_new_balance(self, discord_name, discord_id, price, operation):
        ...
        self.users.update({"coins": int(new_balance)}).filter("discordUser", "eq", discord_name).execute()
    
    def find_or_create_user(self, discord_id, discord_name):
        data = self.users.select("*").filter("discord_id", "eq", discord_id).execute()
        ...
        if user_exists_and_has_id:
            ...
            data = self.users.select("*").filter("discordUser", "eq", discord_name).execute()
            user_exists_but_has_no_id = len(data.data) > 0
            
            if user_exists_but_has_no_id:
                print("Inserting dicord id to existing user.")
                self.users.update({"discord_id": discord_id}).filter("discordUser", "eq", discord_name).execute()
                return
            else:
    ...
    def get_user_coins(self, discord_name, discord_id):
        ...
        coins = self.users.select("coins").filter("discordUser", "eq", discord_name).execute()
        return coins
